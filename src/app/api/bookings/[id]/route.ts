...
import { smsJobAssignment, smsJobRescheduled, smsJobCancelled, smsBookingConfirmation, smsCancellation, smsReschedule } from '@/lib/sms-templates'
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const newCleanerId = ('cleaner_id' in body && body.cleaner_id && body.cleaner_id !== 'undefined') ? body.cleaner_id : null
  if (newCleanerId && newCleanerId !== oldBooking?.cleaner_id) {
    const bookingStartTime = body.start_time || oldBooking?.start_time
    if (bookingStartTime) {
      const bookingDate = bookingStartTime.split('T')[0]
      ...
      const { data: cleaner } = await supabaseAdmin
        .from('cleaners')
        .select('name, unavailable_dates, working_days, schedule')
        .eq('id', newCleanerId)
        .single()

      if (cleaner && !body.force) {
        // Check unavailable_dates
        ...
        // Check working_days
        if (cleaner.working_days && Array.isArray(cleaner.working_days) && cleaner.working_days.length > 0) {
          if (!cleaner.working_days.includes(dayOfWeek)) {
            return NextResponse.json({
  ...
  if ('cleaner_id' in body) updatePayload.cleaner_id = newCleanerId
  ...
  if (dateTimeChanged) {
    ...
    // Emails + SMS
    try {
      // Client reschedule SMS
      if (data.clients?.phone && data.client_id) {
        sendSMS(data.clients.phone, smsReschedule(data), {
          recipientType: 'client',
          recipientId: data.client_id,
          smsType: 'reschedule',
          bookingId: id,
        }).catch(err => console.error('Client reschedule SMS error:', err))
      }
      if (data.cleaners?.email) {
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (cancelSeries && booking.schedule_id) {
    ...
    const [dp, tp] = booking.start_time.split('T')
    const [by, bm, bd] = dp.split('-').map(Number)
    const [bh, bmin] = (tp || '00:00').split(':').map(Number)
    const bookingDate = new Date(by, bm - 1, bd, bh, bmin).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
    const totalCancelled = (cancelled?.length || 0) + 1

    await supabaseAdmin.from('notifications').insert({
      type: 'booking_cancelled',
      title: 'Series Cancelled',
      message: `${booking.clients?.name} - ${booking.recurring_type || 'Recurring'} series cancelled from ${bookingDate} (${totalCancelled} bookings)`,
      booking_id: id
    })

    // Send ONE cancellation email for the series (not per booking)
    try {
      if (booking.clients?.email) {
        const clientEmail = clientCancellationEmail(booking)
        await sendEmail(booking.clients.email, clientEmail.subject, clientEmail.html)
      }
      if (booking.cleaners?.email) {
        const cleanerEmail = cleanerCancellationEmail(booking)
        await sendEmail(booking.cleaners.email, cleanerEmail.subject, cleanerEmail.html)
      }
      if (booking.cleaner_id) {
        await notifyCleaner({
          ...
          type: 'job_cancelled',
          title: 'Series Cancelled',
          message: `${booking.clients?.name} - ${totalCancelled} bookings cancelled from ${bookingDate}`,
          bookingId: id,
      ...
      console.error('Series cancel email error:', e)
    }

    return NextResponse.json({ success: true, cancelled: totalCancelled })
  }
  ...
  if (!skipEmail) {
    try {
      if (booking.clients?.email) {
        const clientEmail = clientCancellationEmail(booking)
        await sendEmail(booking.clients.email, clientEmail.subject, clientEmail.html)
        await supabaseAdmin.from('email_logs').insert({ booking_id: id, email_type: 'cancellation', recipient: booking.clients.email })
      }
      if (booking.cleaners?.email) {
        const cleanerEmail = cleanerCancellationEmail(booking)
        await sendEmail(booking.cleaners.email, cleanerEmail.subject, cleanerEmail.html)
        await supabaseAdmin.from('email_logs').insert({ booking_id: id, email_type: 'cancellation', recipient: booking.cleaners.email })
      }
      ...
      if (booking.clients?.phone && booking.client_id) {
        sendSMS(booking.clients.phone, smsCancellation(booking), {
          ...
          bookingId: id,
        }).catch(err => console.error('Client cancellation SMS error:', err))
      }
      // Push notification to client
