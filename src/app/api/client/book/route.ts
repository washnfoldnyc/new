...
export async function POST(request: Request) {
  try {
    ...
    if (body.date && body.time && !startTime) {
      ...
      endTime = body.date + 'T' + (hour + duration).toString().padStart(2, '0') + ':00:00'
    }
    ...
    const { data, error } = await supabaseAdmin
      ...
      .insert({
        ...
        price: body.price || (body.hourly_rate || 75) * (body.estimated_hours || 2) * 100,
    ...
    try {
      ...
      // Client "request received" email + SMS
      if (data.clients?.email) {
        const receivedEmail = clientBookingReceivedEmail(data)
        await sendEmail(data.clients.email, receivedEmail.subject, receivedEmail.html)
        await supabaseAdmin.from('email_logs').insert({ booking_id: data.id, email_type: 'booking_received', recipient: data.clients.email })
      }
      if (data.clients?.phone && clientId) {
        sendSMS(data.clients.phone, smsBookingReceived(data), {
          recipientType: 'client',
          recipientId: clientId,
          smsType: 'booking_received',
          bookingId: data.id,
        }).catch(err => console.error('Booking received SMS error:', err))
      }
    } catch (emailError) {
